...
namespace IS.uni
{
    public class BUILDING_BUS
    {
        DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
        public BUILDING_BUS()
        {
        }
        public  BUILDING_OBJ createObject()
        {
            BUILDING_OBJ obj = new BUILDING_OBJ();
            return obj;
        }
        public  BUILDING_OBJ createNull()
        {
            return null;
        }
        public List<BUILDING_OBJ> getAll(params spParam[] listFilter)
        {
            List<BUILDING_OBJ> lidata = new List<uni.BUILDING_OBJ>();
            ...
            else
            {
                foreach(DataRow dr in ds.Tables["Tmp"].Rows)
                {
                    ...
                    Type myTableObject = typeof(BUILDING_OBJ);
                    System.Reflection.PropertyInfo[] selectFieldInfo = myTableObject.GetProperties();

                    Type myObjectType = typeof(BUILDING_OBJ.BusinessObjectID);
                    ...
                    foreach (System.Reflection.PropertyInfo info in selectFieldInfo)
                    {
                        ...
                        {
                            //set id value
                            BUILDING_OBJ.BusinessObjectID objid;
                            objid = (BUILDING_OBJ.BusinessObjectID)info.GetValue(obj, null);
        ...
        public BUILDING_OBJ GetByID(BUILDING_OBJ.BusinessObjectID id)
        {
            List<BUILDING_OBJ> li = getAll(new spParam("CODE", SqlDbType.VarChar, id.CODE,0));
        ...
        public string genNextCode(BUILDING_OBJ obj)
        {
        ...
        //public int Insert(BUILDING_OBJ obj)
        //{
        //    int ret = 0;
        //    DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
        //    string sql = "INSERT INTO BUILDING(code,codeview, name, note, edituser,edittime,lock, lockdate) VALUES(@code,@codeview, @name, @note, @edituser,@edittime,@lock, @lockdate)";
        ...
        //    return ret;
        //}
        //public int Update(BUILDING_OBJ obj)
        //{
        //    int ret = 0;
        //    DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
        ...
        //    return ret;
        //}
        public int Delete(BUILDING_OBJ.BusinessObjectID obj)
        {
            int ret = 0;
            DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
            string sql = @"DELETE FROM BUILDING  WHERE code=@code_key
                ";
            SqlCommand com = new SqlCommand();
            com.CommandText = sql;
            com.CommandType = CommandType.Text;
