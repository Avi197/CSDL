...
namespace IS.uni
{
    public class SUBJECTS_BUS
    {
        DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
        public SUBJECTS_BUS()
        {
        }
        public  SUBJECTS_OBJ createObject()
        {
            SUBJECTS_OBJ obj = new SUBJECTS_OBJ();
            return obj;
        }
        public  SUBJECTS_OBJ createNull()
        {
            return null;
        }
        public List<SUBJECTS_OBJ> getAll(params spParam[] listFilter)
        {
            List<SUBJECTS_OBJ> lidata = new List<uni.SUBJECTS_OBJ>();
            ...
            else
            {
                foreach(DataRow dr in ds.Tables["Tmp"].Rows)
                {
                    ...
                    Type myTableObject = typeof(SUBJECTS_OBJ);
                    System.Reflection.PropertyInfo[] selectFieldInfo = myTableObject.GetProperties();

                    Type myObjectType = typeof(SUBJECTS_OBJ.BusinessObjectID);
                    ...
                    foreach (System.Reflection.PropertyInfo info in selectFieldInfo)
                    {
                        ...
                        {
                            //set id value
                            SUBJECTS_OBJ.BusinessObjectID objid;
                            objid = (SUBJECTS_OBJ.BusinessObjectID)info.GetValue(obj, null);
        ...
        public SUBJECTS_OBJ GetByID(SUBJECTS_OBJ.BusinessObjectID id)
        {
            List<SUBJECTS_OBJ> li = getAll(new spParam("CODE", SqlDbType.VarChar, id.CODE,0));
        ...
        public string genNextCode(SUBJECTS_OBJ obj)
        {
        ...
        //public int Insert(SUBJECTS_OBJ obj)
        //{
        //    int ret = 0;
        //    DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
        //    string sql = "INSERT INTO SUBJECTS(code,codeview, name, note, edituser,edittime,lock, lockdate) VALUES(@code,@codeview, @name, @note, @edituser,@edittime,@lock, @lockdate)";
        ...
        //    return ret;
        //}
        //public int Update(SUBJECTS_OBJ obj)
        //{
        //    int ret = 0;
        //    DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
        ...
        //    return ret;
        //}
        public int Delete(SUBJECTS_OBJ.BusinessObjectID obj)
        {
            int ret = 0;
            DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
            string sql = @"DELETE FROM SUBJECTS  WHERE code=@code_key
                ";
            SqlCommand com = new SqlCommand();
            com.CommandText = sql;
            com.CommandType = CommandType.Text;
